...
namespace VpnApp.Windows.Web;

// The Windows build: the app to run when this process is the service, and the UI to show when it
// is the window. Which of the two a run is, the commands and the service itself are the host's.
internal static class App
{
    [STAThread]
    private static int Main(string[] args)
    {
        return WindowsCliHost.Run(args, new WindowsCliHeadParams {
            AppId = AppConstants.AppId,
            LegacyStorageFolderName = null, // no release of this app kept its settings elsewhere
